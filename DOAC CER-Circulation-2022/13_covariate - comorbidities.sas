...
data comp_eff.&out_file;
 	merge seermed.&in_file(keep=PATIENT_ID from_dtm from_dtd from_dty DGNS_CD1-DGNS_CD12 hcpcs_cd in=in1) comp_eff.covariate_10 (keep=patient_id in=in2);
	by patient_id;
	if in1 and in2;
...
data comp_eff.nch_com;
	set comp_eff.nch_com_09-comp_eff.nch_com_16;
	keep PATIENT_ID claim_date anemia asthma COPD dementia gout HL IA Other_IHD Other_CD PUD CR;
run;
proc sort data=comp_eff.nch_com out=comp_eff.nch_com nodupkey;    * 1548476;
	by _all_;
run;
...
data comp_eff.&out_file;
 	merge seermed.&in_file(keep=PATIENT_ID from_dtm from_dtd from_dty DGNS_CD1-DGNS_CD25 hcpcs_cd in=in1) comp_eff.covariate_10 (keep = patient_id in=in2);
	by patient_id;
	if in1 and in2;
...
data comp_eff.outsaf_com;
	set comp_eff.outsaf_com_09-comp_eff.outsaf_com_16;
	keep PATIENT_ID claim_date anemia asthma COPD dementia gout HL IA Other_IHD Other_CD PUD CR;
run;
proc sort data=comp_eff.outsaf_com out=comp_eff.outsaf_com nodupkey;			*  359387;
	by _all_;
run;
...
data comp_eff.&out_file;
 	merge seermed.&in_file(keep=PATIENT_ID ADMSNDTM ADMSNDTD ADMSNDTY DGNSCD1-DGNSCD25 PRCDRCD1-PRCDRCD25 in=in1) comp_eff.covariate_10 (keep = patient_id in=in2);
	by patient_id;
	if in1 and in2;
...
data comp_eff.medpar_com;
	set comp_eff.medpar_com_09-comp_eff.medpar_com_16;
	keep PATIENT_ID claim_date anemia asthma COPD dementia gout HL IA Other_IHD Other_CD PUD CR;
run;
proc sort data=comp_eff.medpar_com out=comp_eff.medpar_com nodupkey;			* 38779;
	by _all_;
run;
...
/* combine the three */
data comp_eff.com;
	set comp_eff.medpar_com comp_eff.nch_com comp_eff.outsaf_com;
run;
proc sort data=comp_eff.com out=comp_eff.com nodupkey;					* 1748455;
	by _all_;
run;
...
data comp_eff.com_2;
	merge comp_eff.com(in=in1) comp_eff.covariate_10(keep=patient_id index_date oneyo in=in2);
	by patient_id;
	if in1 and in2;
...
proc sql;				*;
	...
	create table comp_eff.com_summarized as				
...
data comp_eff.com_final;
	merge comp_eff.com_summarized comp_eff.covariate_10;
	by patient_id;
...
PROC FREQ DATA=comp_eff.com_final;
...
data comp_eff.covariate_11;
	merge comp_eff.covariate_10 comp_eff.com_final;
	by patient_id;
run;
