...
data comp_eff.&out_file;
 	merge seermed.&in_file(keep=PATIENT_ID from_dtm from_dtd from_dty hcpcs_cd DGNS_CD1-DGNS_CD12 in=in1) comp_eff.covariate_05 (keep=patient_id in=in2);
	by patient_id;
	if in1 and in2;
...
data comp_eff.nch_hb;
	set comp_eff.nch_hb_09-comp_eff.nch_hb_16;
	keep PATIENT_ID claim_date HT AKF ALF Stroke bleed alcohol;
run;
proc sort data=comp_eff.nch_hb out=comp_eff.nch_hb nodupkey;    *   1537137;
	by _all_;
run;
...
data comp_eff.&out_file;
 	merge seermed.&in_file(keep=PATIENT_ID from_dtm from_dtd from_dty hcpcs_cd DGNS_CD1-DGNS_CD25 in=in1) comp_eff.covariate_05 (keep=patient_id in=in2);
	by patient_id;
	if in1 and in2;
...
data comp_eff.outsaf_hb;
	set comp_eff.outsaf_hb_09-comp_eff.outsaf_hb_16;
	keep PATIENT_ID claim_date HT AKF ALF Stroke bleed alcohol;
run;
proc sort data=comp_eff.outsaf_hb out=comp_eff.outsaf_hb nodupkey;			*   359165;
	by _all_;
run;
...
data comp_eff.&out_file;
 	merge seermed.&in_file(keep=PATIENT_ID ADMSNDTM ADMSNDTD ADMSNDTY DGNSCD1-DGNSCD25 PRCDRCD1-PRCDRCD25 in=in1) comp_eff.covariate_05 (keep = patient_id in=in2);
	by patient_id;
	if in1 and in2;
...
data comp_eff.medpar_hb;
	set comp_eff.medpar_hb_09-comp_eff.medpar_hb_16;
	keep PATIENT_ID claim_date HT AKF ALF Stroke bleed alcohol;
run;
proc sort data=comp_eff.medpar_hb out=comp_eff.medpar_hb nodupkey;			* 38779;
	by _all_;
run;
...
data comp_eff.&out_file;
 	merge comp_eff.&in_file(keep = PATIENT_ID srvc_mon srvc_day srvc_yr gnn bn in=in1) comp_eff.covariate_05 (keep = patient_id in=in2);
	by patient_id;
	if in1 and in2;
...
%bleed (pdesaf_bleed, pdesaf); * 38987;

data comp_eff.pdesaf_bleed_2;
	set comp_eff.pdesaf_bleed;
...
proc sort data=comp_eff.pdesaf_bleed_2 out=comp_eff.pdesaf_bleed_2 nodupkey;			*38567;
	by _all_;
run;
...
/* combine the four */
data comp_eff.hb;
	set comp_eff.medpar_hb comp_eff.nch_hb comp_eff.outsaf_hb comp_eff.pdesaf_bleed_2;
run;
proc sort data=comp_eff.hb out=comp_eff.hb nodupkey;					*  1770584;
	by _all_;
run;

/* filter for diagnosis one year prior the index_date */
data comp_eff.hb_2;			
	merge comp_eff.hb(in=in1) comp_eff.covariate_05(keep=patient_id index_date oneyo in=in2);
	by patient_id;
	if in1 and in2;
...
proc sql;				*27784;
	...
	create table comp_eff.hb_summarized as				
...
data comp_eff.hb_final;
	merge comp_eff.hb_summarized comp_eff.covariate_05;
	by patient_id;
	keep PATIENT_ID age HT AKF ALF Stroke bleed alcohol;
run;

data comp_eff.hb_score;
	set comp_eff.hb_final;
	hb_score=0;
...
PROC FREQ DATA=comp_eff.hb_score;
    TABLES hb_score HT AKF ALF Stroke bleed alcohol;
RUN;


* Delete datasets no long need;
proc datasets lib=comp_eff nolist;
delete nch_hb_09-nch_hb_16 outsaf_hb_09-outsaf_hb_16 medpar_hb_09-medpar_hb_16 medpar_hb nch_hb outsaf_hb hb hb_2 hb_summarized
		pdesaf_bleed_09-pdesaf_bleed_16 pdesaf_bleed;
run;


/* Final step - merge to cohort */
data comp_eff.covariate_06;
	merge comp_eff.covariate_05(in=in1) comp_eff.hb_score(keep=patient_id hb_score AKF ALF bleed alcohol);
	by patient_id;
	if in1;
run;
